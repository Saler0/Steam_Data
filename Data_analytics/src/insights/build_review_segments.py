...
def _generate_highlights(segments: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    if new_seg and new_seg.get('abandon_rate_30d') and new_seg['abandon_rate_30d'] >= 0.75:
        pct = int(round(new_seg['abandon_rate_30d'] * 100))
        highlights.append(
            f"El abandono post-review es alto en nuevos ({pct}%), indicando fricción de onboarding/rendimiento."
        )
    exp_seg = segments.get('expert')
    vet_seg = segments.get('veteran')
    for seg_name, seg in [('expertos', exp_seg), ('veteranos', vet_seg)]:
        if not seg:
            continue
        pos = seg.get('pos'); abandon = seg.get('abandon_rate_30d')
        if pos is not None and pos < 0.55 and (abandon is None or abandon < 0.3):
            highlights.append(
                f"{seg_name.capitalize()} critican balance/endgame pero mantienen baja tasa de abandono."
            )
    if not highlights and new_seg and new_seg.get('share') and new_seg['share'] > 0.4:
        highlights.append("Los jugadores nuevos representaron la mayoría de reseñas del pico.")
    return highlights
...
def build_review_segments(
    ...
    if events_df.empty:
        raise SystemExit("Eventos vacío; nada que procesar.")
    if reviews_df.empty:
        raise SystemExit("Reseñas vacías; no se pueden generar segmentos.")

    # Preprocesar reseñas
    reviews = reviews_df.copy()
    if 'appid' not in reviews.columns:
        raise SystemExit("El dataset de reseñas debe contener la columna 'appid'.")
    reviews['appid'] = reviews['appid'].astype(str)
    if 'review_date' not in reviews.columns:
        raise SystemExit("El dataset de reseñas debe contener 'review_date'.")
    ...
    for _, event in events_df.iterrows():
        ...
        if peak_dt is None:
            # intentar con year_month como primer día del mes
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Construye segmentos de reseñas por pico")
    ap.add_argument('--events', required=True, help='Parquet/CSV con eventos detectados (outputs/events/events.parquet).')
    ap.add_argument('--reviews', required=True, help='Dataset a nivel reseña con métricas necesarias.')
    ap.add_argument('--topics', default=None, help='Dataset opcional con tópicos BERTopic por reseña.')
    ap.add_argument('--window-before', type=int, default=30, help='Días antes del pico a considerar (default: 30).')
    ap.add_argument('--window-after', type=int, default=15, help='Días después del pico (default: 15).')
...
def main() -> None:
    ...
    if result_df.empty:
        print("[WARN] No se generaron segmentos de reseñas (sin reseñas dentro de las ventanas definidas).")
    ...
    print(f"[OK] Segmentos de reseñas -> {out_path}")
