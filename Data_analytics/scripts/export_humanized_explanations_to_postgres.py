#!/usr/bin/env python
"""Exporta el fichero de explicaciones humanizadas a PostgreSQL."""
...
def export_to_postgres(
    ...
    df = pd.read_csv(p, engine='python') # Usar engine python por robustez
    if df.empty:
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Exporta explicaciones humanizadas a PostgreSQL.")
    ap.add_argument(
        "--in",
        dest="csv_path",
        default="outputs/events/humanized_explanations.csv",
        help="Ruta al CSV con las explicaciones humanizadas.",
    )
    ap.add_argument("--table", default="humanized_explanations", help="Nombre de la tabla de destino en PostgreSQL.")
